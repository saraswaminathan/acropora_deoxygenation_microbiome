...
## Read in data and filter out partial days

```{r}
# Load data frame
YSIfiltered <- read_xlsx("data/sonde_data_withSeptNurseryMinidot.xlsx") %>%
  ...
  mutate(fulldays = factor(ifelse(date >= "2019-06-12" & date <= "2019-07-07", "JJ", #june-july deployment dates
                       ifelse(date >= "2019-07-10" & date <= "2019-08-04", "JA", #july-august deployment dates
                          ifelse(date >= "2019-08-07" & date <= "2019-09-08", "AS", #august-september deployment dates
                                  ifelse(date >= "2019-09-10" & date <= "2019-10-07", "SO", NA)))))) %>% #september-october deployment dates, NA for partial days
  mutate(fulldays = factor(fulldays, levels = c("JJ", "JA", "AS", "SO"))) %>%
  filter(!is.na(fulldays)) 
```

## calculate moving averages of Temp, DO, Turbidity, TDS, pH, and Salinity
...
YSI_ma <- YSIfiltered %>%
  ...
  mutate(maTemp_24 = rollmean(Temp_degC, k = 144, fill = NA),  # 24-hr temp running average (24 hrs * 60 min = 1440. Divided by measurements every 10 minutes.)
    ...
    maTurb_24 = rollmean(Turb_FNU, k = 144, fill = NA), # 24-hr Turbidity running average
    maTDS_24 = rollmean(TDS_mgL, k = 144, fill = NA), # 24-hr TDS running average
    mapH_24 = rollmean(pH, k = 144, fill = NA), # 24-hr pH running average
    maSal_24 = rollmean(Sal_PSU, k = 144, fill = NA)) # 24-hr Salinity running average

# separate dataframe for munson (inshore) data
ma_munson <- YSI_ma %>%
  filter(site == "munson") 

# separate dataframe for nursery (offshore) data
ma_nursery <- YSI_ma %>%
  filter(site == "nursery")
...
## Munson (inshore)
```{r}
#calculate mean temp +/- SE
ma_munson%>%
  filter(!is.na(maTemp_24)) %>%
  summarise(N = n_distinct(date),
            mean = mean(maTemp_24),
            SD = sd(maTemp_24),
            SE = SD/sqrt(N))

# Calculate number of days where daily mean was greater than 31 degrees C at munson
ma_munson%>%
  ungroup()%>%
  dplyr::group_by(date)%>%
...
# find number of instances of DO less than 4mg/L at munson
ma_munson%>%
  filter(DOmgL<4)


rangeMunson <- ma_munson%>%
  group_by(date) %>%
  summarise(N = n_distinct(date),
    min = min(Temp_degC), 
            max = max(Temp_degC),
            range = max - min)%>%
  mutate(meanRange = mean(range),
    SD = sd(range), 
    SE = SD/sqrt(N))
```
...
## set plotting colors for figures
```{r}
...
(tds_sal_compare<- ggplot(data = ysiSumm, aes(x = maTDS_24, y = maSal_24, color = site))+
  scale_color_manual(values=siteColorsLine) +
  geom_point(alpha = 0.01))
ggsave(file = "fig/tds_sal_compare.pdf", plot = tds_sal_compare, width = 8, height = 6)
...
## Temperature models
- temp_brm5 is the best model
- uncomment to run models

```{r}
hist(YSI_ma$maTemp_24) # visualize data
hist(log(YSI_ma$maTemp_24)) # visualize data (log-transformed)
########################################################################################################################################
...
# ###########################################################################################################################################
# trying as a mixture model - did not converge 
##### Set priors
